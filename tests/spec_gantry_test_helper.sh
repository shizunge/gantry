...
export MUST_BE_A_NUMBER="must be a number. Got \".*\""
...
export SKIP_REASON_PREVIOUS_ERRORS="due to previous [0-9]+ error\(s\)"
...
export AUTOMATICALLY="automatically"
export SPECIFIED_BY_USER="specified by user"
export REPLICATED_MODE_NO_RUNNING_TASK="is in replicated mode and has no running task"
...
export NUM_SERVICES_SKIP_JOBS="Skip updating [0-9]+ service\(s\) because they are job\(s\)"
...
export FAILED_TO_REMOVE_IMAGES="Failed to remove images. The global job .* exited with code [0-9]+"
...
export NUM_SERVICES_INSPECT_FAILED="[0-9]+ service\(s\) inspection failed"
export NUM_SERVICES_ERRORS="There are [0-9]+ error\(s\)"
...
reset_gantry_env() {
  ...
  local CLEANUP_IMAGES_REMOVER
  CLEANUP_IMAGES_REMOVER=$(_get_gantry_image "${SUITE_NAME}")
  export GANTRY_CLEANUP_IMAGES_REMOVER="${CLEANUP_IMAGES_REMOVER}"
