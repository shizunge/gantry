...
Describe 'service-single-service'
  ...
  Describe "test_new_image_SERVICES_SELF"
    ...
    End
  End
  Describe "test_new_image_update_twice"
    TEST_NAME="test_new_image_update_twice"
    IMAGE_WITH_TAG=$(get_image_with_tag "${SUITE_NAME}")
    SERVICE_NAME=$(get_test_service_name "${TEST_NAME}")
    test_new_image_update_twice() {
      local TEST_NAME="${1}"
      local SERVICE_NAME="${2}"
    ...
    AfterEach "common_cleanup ${TEST_NAME} ${IMAGE_WITH_TAG} ${SERVICE_NAME}"
    It 'run_test'
      When run test_new_image_update_twice "${TEST_NAME}" "${SERVICE_NAME}" "${IMAGE_WITH_TAG}"
      The status should be success
      The stdout should satisfy display_output
      The stdout should satisfy spec_expect_no_message ".+"
      The stderr should satisfy display_output
